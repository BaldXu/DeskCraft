...
/// 数字时钟配置的持久化与 widget 桥同步。
///
/// - App 侧：SharedPreferences 存 JSON（[appConfigKey]）+ 最后刷新时间戳；
...
class ClockConfigStore {
  ...
  static const String widgetDataJsonKey = 'clock_config_json';

  static const String _appConfigKey = 'clock_config_v1';
  static const String _lastSyncKey = 'clock_last_sync_ms';
  ...
  static Future<ClockConfig> load() async {
    ...
    return ClockConfig.fromJsonString(sp.getString(_appConfigKey));
  }

  /// 保存配置并推送到桌面 widget（数据桥：saveWidgetData → updateWidget）。
  /// 返回 updateWidget 的结果（true = 已通知 provider 重绘）。
  static Future<bool> saveAndPush(ClockConfig config) async {
